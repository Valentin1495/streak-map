...
import { FlatList, Image, ScrollView, StyleSheet, Text, TouchableOpacity, View, Dimensions, Alert } from 'react-native';
...
function CalendarView({ photos, streak, onPhotoPress, onRecordsPress, onPlacesPress }: CalendarViewProps) {
      ...
      <View style={calStyles.grid}>
        {cells.map((day, idx) => {
          ...
          const dayContent = (
            ...
            >
              <Text style={[calStyles.dayNum, hasPhoto && calStyles.dayNumActive]}>{day}</Text>
            </View>
          );
...
export function RecordView({ photos, onPhotosChange, streak }: RecordViewProps) {
  ...
  const [photoSheetReturnPlaceGroup, setPhotoSheetReturnPlaceGroup] = useState<PlaceGroup | null>(null);
  const [pendingPhotoSheetReturnPlaceGroup, setPendingPhotoSheetReturnPlaceGroup] = useState<PlaceGroup | null>(null);
  ...
  const sortedPhotos = useMemo(() => [...photos].sort((a, b) => b.taken_at.localeCompare(a.taken_at)), [photos]);
  ...
  const closePhotoSheet = () => {
    setPhotoSheetReturnPlaceGroup(null);
    setPendingPhotoSheetReturnPlaceGroup(null);
    setSelectedPhoto(null);
  };

  const returnToPlaceStatsSheet = () => {
    if (photoSheetReturnPlaceGroup == null) return;
    setPendingPhotoSheetReturnPlaceGroup(photoSheetReturnPlaceGroup);
    setSelectedPhoto(null);
  };
  ...
  const handleStatsPhotoPress = (photo: Photo) => {
    ...
    setPhotoSheetReturnPlaceGroup(statsSheetMode === 'places' ? selectedPlaceGroup : null);
  ...
  const handleDelete = () => {
    ...
    Alert.alert('사진을 삭제할까요?', '삭제한 사진은 복구할 수 없어요.', [
    ...
    <View style={styles.container}>
      ...
      <View style={styles.segmentRow}>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.segmentText, mode === 'map' && styles.segmentTextActive]}>🗺 지도 보기</Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.segmentText, mode === 'calendar' && styles.segmentTextActive]}>📅 캘린더 보기</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <View style={styles.statsSheetHeader}>
          ...
          <Button type="dark" style="weak" size="tiny" onPress={closeStatsSheet}>
            닫기
          </Button>
        </View>
        ...
        {statsSheetMode === 'records' && selectedRecordPhoto != null ? (
          ...
          >
            <TouchableOpacity onPress={() => setFullPhoto(selectedRecordPhoto)} activeOpacity={1}>
              <Image
            ...
            <View style={styles.recordDetailBody}>
              ...
              <Text
                style={[styles.recordDetailMemo, getVisibleText(selectedRecordPhoto.memo) == null && styles.emptyMemo]}
              >
                {getVisibleText(selectedRecordPhoto.memo) ?? '남긴 메모가 없어요.'}
              </Text>
            </View>
          </ScrollView>
        ) : statsSheetMode === 'records' ? (
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                <Image source={{ uri: getPhotoUrl(item.storage_path) }} style={styles.recordThumb} resizeMode="cover" />
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.recordTexts}>
                    ...
                    <Text style={styles.recordMeta}>{item.photos.length}개의 기록</Text>
                    <Text style={styles.recordMemo} numberOfLines={1}>
                      {formatKoreanDate(firstPhoto.streak_date)}
                    </Text>
                  </View>
                </TouchableOpacity>
              );
      ...
      <BottomSheetModal
        ...
        onExitComplete={() => {
          ...
          setDisplayGroup([]);
          if (pendingPhotoSheetReturnPlaceGroup != null) {
            setSelectedPlaceGroup(pendingPhotoSheetReturnPlaceGroup);
            setStatsSheetMode('places');
            setPendingPhotoSheetReturnPlaceGroup(null);
            setPhotoSheetReturnPlaceGroup(null);
          }
        ...
        {displayPhoto != null && (
          <View style={styles.photoSheetContent}>
            ...
            <View style={styles.detailImageWrapper}>
              <TouchableOpacity onPress={() => setFullPhoto(displayPhoto)} activeOpacity={1}>
                <Image
            ...
            <View style={styles.detailBody}>
              <View style={styles.detailInfo}>
                ...
                <Text style={[styles.detailMemo, getVisibleText(displayPhoto.memo) == null && styles.emptyMemo]}>
                  {getVisibleText(displayPhoto.memo) ?? '남긴 메모가 없어요.'}
                </Text>
              </View>
              ...
              <View style={styles.actionGroup}>
                ...
                {photoSheetReturnPlaceGroup != null && (
                  <Button
                    type="dark"
                    style="weak"
                    size="medium"
                    display="full"
                    onPress={returnToPlaceStatsSheet}
                    containerStyle={styles.actionButtonContainer}
                  >
                    돌아가기
                  </Button>
      ...
      <FullPhotoModal
        ...
        uri={fullPhoto == null ? null : getPhotoUrl(fullPhoto.storage_path)}
        placeName={fullPhoto?.place_name}
